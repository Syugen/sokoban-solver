...
class Puzzle:
  def __init__(self, state):
    ...
    self.state = state
    self.dist_maps = {}
    for tar in state.storage:
      self.dist_maps[tar] = np.array([[-1] * state.height] * state.width)
      self.dist_maps[tar][tar[0]][tar[1]] = 0
      
  
  def __eq__(self, other):
    return self.state.width == other.state.width and\
           self.state.height == other.state.height and\
           self.state.storage == other.state.storage and\
           self.state.obstacles == other.state.obstacles and\
           self.state.restrictions == other.state.restrictions and\
           self.state.box_colours == other.state.box_colours and\
           self.state.storage_colours == other.state.storage_colours
  
  def map_set(self, tar, col, row, value):
    self.dist_maps[tar][col][row] = value
    self.n += 1
...
#IMPLEMENT
    ...
    puzzle = None
    this_puzzle = Puzzle(state)
    for cur_puzzle in PUZZLES:
      if cur_puzzle == this_puzzle:
        puzzle = cur_puzzle
        break
    if puzzle is None: 
      PUZZLES.insert(0, this_puzzle)
      puzzle = this_puzzle
    ...
    for box in boxes:
      ...
      for tar in tars:
        if res and tar not in res[state.boxes[box]]:
          ...
          costs[i][j] = get_box_storage_distance(puzzle, box, tar)
        j += 1
      i += 1
    ...
    if any(num >= 16 for num in num_obs): return 10e8
    j = 0
    for tar in tars:
    ...
    #print(puzzle.n)
    #[print(dm, '\n', puzzle.dist_maps[dm].T)for dm in puzzle.dist_maps]
    #print(costs)
    #print(tars,num_obs)
...
def reachable_direction(state, col, row):
  ...
  #print((col, row), L, R, U ,D)
  return sum([L * 5 or LL * 4, R * 5 or RR * 4, U * 5 or UU * 4, D * 5 or DD * 4])

def get_box_storage_distance(puzzle, pos, target):
  state = puzzle.state
  pos_col, pos_row = pos
  tar_col, tar_row = target
  
  if puzzle.dist_maps[target][pos_col][pos_row] != -1:
    return puzzle.dist_maps[target][pos_col][pos_row]
  ...
  for succ in seq:
    if succ == LEFT and (succ[0] < 0 or succ in state.obstacles or RIGHT[0] >= state.width or RIGHT in state.obstacles) or\
      ...
      sub = 10e8
    else: sub = get_box_storage_distance(puzzle, succ, target)
    if sub < puzzle.dist_maps[target][pos_col][pos_row] or puzzle.dist_maps[target][pos_col][pos_row] == -1:
      puzzle.map_set(target, pos_col, pos_row, min(10e8, sub + 1))
  return puzzle.dist_maps[target][pos_col][pos_row]
