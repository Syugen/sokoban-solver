...
#IMPLEMENT
    ...
    for box in boxes:
      #print("box is", box)
      if is_on_chamber_wall(state, box[0], box[1]): 
        #print("bye")
        return 10e8
      
      # Other checks only works if box not on storage
    ...
    for i in range(len(num_obs)):
      if num_obs[i] >= 16 and tars[i] not in boxes:
        return 10e8
    
    j = 0
    for tar in tars:
      # Bonus for moving closer to storage with highest surronding obstacles
      bonus_weight = max(0, 10 - (max(num_obs) - num_obs[j])) / 5
      if not res:
        bonus += max(0, len(tars) - min(costs[:, j])) * bonus_weight
      else:
        priority_box_index = [tar in r for r in res].index(True)
        for box in boxes:
          if state.boxes[box] == priority_box_index:
            priority_box = box
            break
        bonus -= get_box_storage_distance(state, box, tar, []) * bonus_weight
...
def is_on_chamber_wall(state, col, row):
  #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  L, R, U, D, LL, RR, UU, DD = surrounding_obstacles(state, col, row, True)
  box = (col, row)
  been = [box]
  if (L or LL) and (R or RR):
    seq = [(col, row - 1), (col, row + 1)]
  elif (U or UU) and (D or DD):
    seq = [(col - 1, row), (col + 1, row)]
  else: return False
  
  for col, row in seq:
    tars = []
    rs = is_in_chamber(state, col, row, been, tars)
    
    if rs: return True
  return False


def is_in_chamber(state, col, row, been, tars):
  #print(col, row)
  if (col < 0 or col >= state.width or row < 0 or row >= state.height or\
      (col, row) in state.obstacles or (col, row) in been):
    #print(col, row, "t")
    return len(been) > 1
  if (col, row) == state.robot: return False
  if (col, row) in state.storage:
  ...
  been.append((col, row))
  succ = [(col - 1, row), (col + 1, row), (col, row - 1), (col, row + 1)]
  return all([is_in_chamber(state, col, row, been, tars) for col, row in succ])

def box_in_corner(state, col, row):
  L, R, U, D, LL, RR, UU, DD = surrounding_obstacles(state, col, row, True)
  LR = (L or LL or R or RR)
  UD = (U or UU or D or DD)
  if LR and UD: return True # Box in corner
  ...
  RB = col < state.height and (col + 1, row) in state.boxes and\
    (row == 0 or row == state.height - 1 or\
     (col + 1, row - 1) in state.obstacles or (col + 1, row + 1) in state.obstacles) and\
    ((col + 1, row) not in state.obstacles or (col, row) not in state.obstacles)  
  if LR and (UB or DB) or UD and (LB or RB): return True
  return False
